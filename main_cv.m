...
%%%CV_type
if flag==1
    cv_data = cross_validation1(inter_tensor,k_folds,seed);
    result = five_cross1(DATA,cv_data,option);
    LTDSSL_type = result;
    save LTDSSL_type LTDSSL_type;
else
    cv_data = cross_validation2(inter_tensor,k_folds,seed);
    result = five_cross2(DATA,cv_data,option);
...
%%%perform cv_type
function result = five_cross1(DATA,cv_data,option)
...
for k=1:k_folds
    ...
    %    
    resultk =  evaluate_opt2(pre_tensor,real_tensor,cv_data{k,2});    
...
%%%perform cv_triplet
function result = five_cross2(DATA,cv_data,option)
cv = length(cv_data);
result = 0;
FH = {DATA.human_paac_fea,DATA.human_ctd_fea};
FV = {DATA.virus_paac_fea,DATA.virus_ctd_fea};
...
for k=1:cv
    ...
    for i=1:length(seeds)
        ...
        resultk = [resultk;evaluate_opt1(pre_tensor(ind_test),label)];
    end
...
%k_folds=5,five fold cross
%prop: Ratio of positive and negative cases in the test set
function cv_data = cross_validation1(inter_tensor,k_folds,seed)
...
%prop: Ratio of positive and negative cases in the test set
function cv_data = cross_validation2(inter_tensor,cv,seed)
cv_data = cell(cv,2);  
...
function resultk =  evaluate_opt2(pre_tensor,real_tensor,pos_ind)
